...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    switch (req.method) {
        case 'POST':
            await createStudentAnswer(req, res)
            break
        default:
...
async function createStudentAnswer(req: NextApiRequest, res: NextApiResponse) {
    let studentAnswer = {}
    try {
        ...
        await addDoc(getCollection(CollectionType.ERROR), {
            ...
            studentAnswer,
            occurAt: new Date()})
        res.status(400).json({message: 'Your answer has not been submitted but logged. Please contact Ms. Lien for more information.'})
    }
